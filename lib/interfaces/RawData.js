...
class RawData extends MultidimensionalInterface {
    ...
    constructor(config, commMan) {
        ...
        this._baseUri = this._serverUrl + this._name;
    ...
    setupPollInterfaces() {
        ...
        super.commMan.router.get('/' + this.name + '/latest', async (ctx, next) => {
            ...
            if (self._latest.length === 0) {
                ...
                if (ifNoneMatchHeader && ifNoneMatchHeader === etag) {
                    ...
                    ctx.response.body = this.prevResponseBody;
                } else {
                    ctx.response.set({
        ...
        super.commMan.router.get('/' + this.name + '/fragments', async (ctx, next) => {
            ...
            if (queryTime.toString() === 'Invalid Date') {
                ...
                ctx.status = 302
                ctx.redirect(this._baseUri + '/fragments?time=' + new Date().toISOString());
                return;
            }
            ...
            if (queryTime.getTime() !== fragment.getTime()) {
                ...
                ctx.status = 302
                ctx.redirect(this._baseUri + '/fragments?time=' + fragment.toISOString());
                return;
            }
            ...
            let uri = this._baseUri + '/fragments?time=' + fragment.toISOString(); 
            let metaData = await this.createMetadata(uri, index);
    ...
    async createMetadata(subject, index) {
       let quads = [];
        ...
        quads.push(quad(
            namedNode(subject + '#Dataset'),
            namedNode('http://www.w3.org/2000/01/rdf-schema#type'),
            namedNode('http://rdfs.org/ns/void#Dataset'),
            namedNode('#Metadata')
        ));
        quads.push(quad(
            namedNode(subject + '#Dataset'),
            namedNode('http://purl.org/dc/terms/subject'),
            namedNode('http://dbpedia.org/resource/Signal_timing'),
            namedNode('#Metadata')
        ));
        quads.push(quad(
            namedNode(subject + '#Dataset'),
            namedNode('http://purl.org/dc/terms/subject'),
            namedNode('http://dbpedia.org/resource/Traffic_light'),
            namedNode('#Metadata')
        ));
        quads.push(quad(
            namedNode(subject + '#Dataset'),
    ...
    get prevResponseBody() {
        return this._prevResponseBody;
    }
